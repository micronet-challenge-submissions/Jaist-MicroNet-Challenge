...
FullyConnected = collections.namedtuple(
    'FullyConnected', ['kernel_shape', 'use_bias', 'activation','tied_weights'],
    defaults=(0,True,None,None)
    )

QRNN = collections.namedtuple(
  'LSTM',['input_size','hidden_size','window','output_gate']
)

Embedding = collections.namedtuple(
  'Embedding',['input_size','n_channels']
)

GlobalMax = collections.namedtuple(
  'GlobalMax',['input_size','n_channels']
)
...
def get_flops_per_activation(activation):
  ...
  if activation == 'relu':
    ...
    return 2, 1
  elif activation == 'tanh': # Tanh: (exp(x)-exp(-x))/(exp(x)+exp(-x))
    return 3, 2
  else:
...
def count_ops(op, sparsity, param_bits):
  ...
  if isinstance(op, Conv2D):
  ...
  elif isinstance(op, QRNN):
    # X = (seq_len, batch_size, k * hidden) window=k 
    # Y = nn.Linear(self.window * self.input_size, 3 * self.hidden_size if self.output_gate else 2 * self.hidden_size)(X)
    # Z, F, O = Y.chunk(3, dim=2) if output_gate else Y.chunk(2, dim=2) + [None]
    # Z = torch.nn.functional.tanh(Z)
    # F = torch.nn.functional.sigmoid(F)
    # C = ForgetMult()(F, Z, hidden, use_cuda=self.use_cuda) h_t = f_t * z_t + (1 - f_t) * h_{t-1}
    # H = torch.nn.functional.sigmoid(O) * C if output_gate else C
    c_in = op.window * op.input_size
    c_out = op.hidden_size * (3 if op.output_gate else 2)
    ...
    # QRNN always uses bias terms
    param_count += c_out * param_bits
    flop_adds += c_out

    # tanh, sigmoid x 2
    n_muls, n_adds = get_flops_per_activation('tanh')
    flop_mults += n_muls * op.hidden_size 
    flop_adds += n_adds * op.hidden_size

    n_muls, n_adds = get_flops_per_activation('sigmoid')
    flop_mults += n_muls * op.hidden_size * (2 if op.output_gate else 1)
    flop_adds += n_adds * op.hidden_size * (2 if op.output_gate else 1)
    
    # C = ForgetMult()(F, Z, hidden, use_cuda=self.use_cuda) h_t = f_t * z_t + (1 - f_t) * h_{t-1}
    flop_mults +=  op.hidden_size * 2
    flop_adds += op.hidden_size * 2

  elif isinstance(op, Embedding):
    param_count += op.input_size * op.n_channels * param_bits
  elif isinstance(op, GlobalMax):
    flop_mults += op.input_size * op.n_channels
  else:
    raise ValueError('Encountered unknown operation %s.' % str(op))
  if getattr(op,'tied_weights',False):
    param_count = 0 # the paramters are tied/counted to other module already.
...
def get_info(op):
  ...
  if isinstance(op, (DepthWiseConv2D, Conv2D)):
    ...
    input_size = 1
  elif isinstance(op,(Embedding,QRNN,GlobalMax)):
    pass
  else:
    raise ValueError('Encountered unknown operation %s.' % str(op))
